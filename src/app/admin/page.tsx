...
export default async function AdminPage() {
  const {
    ...
    totalRefunds,
    refundPurchases,
    averageNetPurchasesPerCustommer,
    ...
    <div className="container my-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 md:grid-cols-4 gap-4">
        ...
        <StatCard title="Refunded Purchases">
          {formatNumber(refundPurchases)}
        </StatCard>
        <StatCard title="Purchases Per User">
          {formatNumber(averageNetPurchasesPerCustommer, {
...
async function getPurchaseDetails() {
  ...
  const refundPurchases = refundData?.totalPurchases ?? 0;
  const averageNetPurchasesPerCustommer =
